...
標準の `Wire.h` のみ使用（ESP32版）。

> ⚠️ **Pico W 版の追加ライブラリ:**
> - `SerialBT.h` — Bluetooth SPP 通信用のカスタムライブラリ（imo Lab. の頒布データに含まれる）
> - Pico W 内蔵 BT を使う場合は、このライブラリの代わりに `SerialBluetooth` 等の
>   Pico W 対応ライブラリへの差し替えが必要
> - Bluetooth を使わずに USB シリアルのみで運用する場合は、`SerialBT` 関連コードを
>   `Serial` に置き換えることで対応可能
